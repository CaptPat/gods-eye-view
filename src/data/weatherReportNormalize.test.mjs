...
test('queries are validated and snapped to the 0.05° grid', () => {
  ...
  assert.match(
    parseReportQuery(new URLSearchParams({ lon: '1' })).error,
    /lat/,
  );
...
test('upstream URLs carry the documented parameters', () => {
  ...
  assert.equal(
    new URL(googleHourlyUrl(POINT, 'K', 'fixture-page-2')).searchParams.get(
      'pageToken',
    ),
    'fixture-page-2',
  );
...
test('hourly pages are joined and capped at 48 entries', () => {
  const pages = [
    fixture('google-hourly-page1.json'),
    fixture('google-hourly-page2.json'),
  ];
  ...
  assert.deepEqual(hourly[47], {
    time: Date.parse('2026-09-16T11:00:00Z'),
...
test('marine is null inland; solar reads current radiation and surface temperature', () => {
  ...
  assert.deepEqual(
    { ...coastal, dailyMaxWaveM: coastal.dailyMaxWaveM.slice(0, 1) },
    {
      waveHeightM: 0.46,
      wavePeriodS: 4,
      waveFromDeg: 165,
