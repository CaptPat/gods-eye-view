...
test('a RainViewer manifest becomes sorted frames with server-side paths', () => {
  ...
  assert.equal(
    normalizeRainViewerManifest({
      host: 'http://insecure.example',
      radar: { past: [] },
    }),
    null,
  );
  assert.equal(
    normalizeRainViewerManifest({ host: 'https://tilecache.rainviewer.com' }),
    null,
  );
});
...
test('IEM frames are 13 ten-minute boundaries ending at least five minutes ago', () => {
  ...
  for (let i = 1; i < times.length; i += 1)
    assert.equal(times[i] - times[i - 1], FRAME_STEP_MS);
  assert.equal(
    iemFrameTimes(Date.UTC(2026, 8, 14, 4, 54, 59)).at(-1),
    Date.UTC(2026, 8, 14, 4, 40),
  );
});
...
test('IEM queries are validated for time, bbox and size', () => {
  const ok = new URLSearchParams({
    time: '2026-09-14T04:50:00Z',
    bbox: '-100,28,-94,34',
    width: '256',
    height: '256',
  });
  assert.deepEqual(parseIemQuery(ok, NOW), {
    time: '2026-09-14T04:50:00Z',
    bbox: [-100, 28, -94, 34],
    width: 256,
    height: 256,
  ...
  const bad = (patch) =>
    parseIemQuery(
      new URLSearchParams({ ...Object.fromEntries(ok), ...patch }),
      NOW,
    );
  assert.match(
    bad({ time: '2026-09-14T04:52:00Z' }).error,
    /time/,
    'not a 5-minute boundary',
  );
  assert.match(
    bad({ time: '2026-09-13T20:00:00Z' }).error,
    /time/,
    'older than 6 hours',
  );
  assert.match(
    bad({ time: '2026-09-14T05:05:00Z' }).error,
    /time/,
    'in the future',
  );
  assert.deepEqual(
    bad({ bbox: '-135,45,-112.5,67.5' }).bbox,
    [-135, 45, -112.5, 67.5],
    'a level-3 edge tile is accepted',
  );
  assert.match(
    bad({ bbox: '-140,28,-94,34' }).error,
    /bbox/,
    'west beyond the envelope',
  );
  assert.match(
    bad({ bbox: '-100,28,-40,34' }).error,
    /bbox/,
    'east beyond the envelope',
  );
  assert.match(
    bad({ bbox: '-130,21,-50,50' }).error,
    /bbox/,
    'span wider than 70 degrees',
  );
  assert.match(
    bad({ bbox: '-94,28,-100,34' }).error,
    /bbox/,
    'west not less than east',
  );
