...
const REPORT = Object.freeze({
  ...
  hourly: [
    {
      time: Date.parse('2026-09-14T12:00:00Z'),
      condition: 'Clear',
      iconType: 'CLEAR',
      temperatureC: 29.8,
      precipChancePct: 0,
  ...
  daily: [
    {
      ...
      lowC: 29.4,
      precipChancePct: 45,
      sunrise: Date.parse('2026-09-14T12:03:33.388Z'),
...
const view = (patch = {}, options = {}) =>
  buildReportView(
    { ...REPORT, ...patch },
    { units: 'imperial', now: T0, ...options },
  );
const rows = (list) =>
  Object.fromEntries(list.map((row) => [row.label, row.value]));
...
test('per-section status text, stale line, missing place and missing time zone', () => {
  const googleDown = view({
    sources: { ...REPORT.sources, google: 'unavailable' },
    now: null,
    hourly: null,
    daily: null,
    timeZone: null,
  ...
  assert.deepEqual(googleDown.pin, {
  ...
  assert.equal(
    view({
      sources: { ...REPORT.sources, google: 'not-configured' },
      now: null,
      hourly: null,
      daily: null,
    }).sections.forecast,
  ...
  const marineDown = view({
    sources: { ...REPORT.sources, openMeteoMarine: 'unavailable' },
    marine: null,
  });
  ...
  const solarDown = view({
    sources: { ...REPORT.sources, openMeteoSolar: 'unavailable' },
    solar: null,
  });
  assert.equal(solarDown.sections.solar, 'Open-Meteo unavailable');
  assert.deepEqual(rows(solarDown.solar.rows), {
    Sunrise: '07:03',
    Sunset: '19:25',
  });

  assert.equal(
    view({ stale: true }, { now: T0 + 45 * 60_000 }).status,
    'Showing weather from 45 min ago',
  );
  assert.equal(view({ place: null }).title, '29.250, -94.800');
  assert.equal(
    view({ now: { ...REPORT.now, humidityPct: null } }).now.grid[0].value,
    '—',
  );
});
