...
test('the panel is inserted first in the rail with the shared panel markup and permanent credits', (t) => {
  ...
  s.panel.showLoading({
    title: '29.250, -94.800',
    coordinates: '29.250, -94.800',
  });
  assert.deepEqual(s.texts('.weather-report-status'), ['Loading weather']);
  assert.equal(
    s.el.querySelector('.weather-report-status').getAttribute('aria-live'),
    'polite',
  );
...
test('a full view renders every section in order', (t) => {
  ...
  assert.deepEqual(s.texts('.weather-report-updated'), [
    '29.250, -94.800 · Updated 07:10 local (UTC−5)',
  ]);
  assert.deepEqual(s.texts('.weather-report-status'), ['']);
  assert.deepEqual(s.texts('.weather-report-now-temp'), ['86°F']);
  assert.deepEqual(s.texts('.weather-report-now-condition'), [
    'Sunny · Feels like 98°F',
  ]);
  assert.ok(s.texts('dd').includes('30.05 inHg'));
  assert.deepEqual(s.texts('.weather-report-hour'), [
    '07:00Clear86°F0%SSE 8 mph',
  ]);
  assert.deepEqual(s.texts('.weather-report-day'), [
    'Mon 14Sunny / Clear89°F / 85°F45%',
  ]);
...
test('section statuses, inland marine and stale reports', (t) => {
  ...
  assert.deepEqual(s.texts('.weather-report-section-status'), [
    'Google forecast unavailable',
    'Open-Meteo unavailable',
  ]);
  assert.equal(
    s.el
      .querySelector('.weather-report-section-status')
...
test('units, refresh, close, errors and destroy', (t) => {
  ...
  s.panel.render(viewOf());
  s.panel.showError('Weather sources unavailable', {
...
test('place names are rendered as text, never markup', (t) => {
  ...
  assert.deepEqual(s.texts('.weather-report-place'), [
