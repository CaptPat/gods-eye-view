...
// ── Google refuses: the search falls back to OpenStreetMap ──────────────────
// A key whose Cloud project lacks the Geocoding API answers REQUEST_DENIED for
// every query (observed 2026-09-13), and Places answers 403. Before the fallback
// the search box toasted "Location not found" for any place at all.
const OSM_DUBAI_RESULT = {
...
test('a search Google refuses still flies, using the OpenStreetMap result', async () => {
  const viewer = stubViewer();
  const requested = [];
  const hadWindow = Object.hasOwn(globalThis, 'window');
  const priorWindow = globalThis.window;
  ...
  globalThis.fetch = async (url) => {
    ...
    requested.push(href);
    if (href.includes('maps.googleapis.com/maps/api/geocode/json')) {
    ...
    if (href.startsWith('/api/geocode/search')) {
      return reply(200, { status: 'OK', source: 'openstreetmap', results: [OSM_DUBAI_RESULT] });
    }
    if (href.startsWith('/api/google/text-search')) {
      return reply(403, { error: 'Requests to this API places.googleapis.com method google.maps.places.v1.Places.SearchText are blocked.', places: [] });
    }
