...
| R14 | Rate limit: 60 requests/min per client, 240/min global, one limiter for all JSON routes | No tile routes. The global cap keeps the worst case (3 upstream calls per tide report) well under the ~600/min burst that NOAA blocked |
