...
`gods-eye-view/search` exports an explicit geocoding service and Google, Nominatim
and Photon adapters. The entry owns normalization, bounded caches, deadlines and fallback
...
keyless Nominatim fallback, which calls the local `/api/geocode/search` route
(`server/providers/geocode.js`). The Photon adapter remains exported but is not
composed. The application passes this service to location
