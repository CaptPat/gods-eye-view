...
| **OpenStreetMap (Nominatim)** | Reverse-geocoded place label in the cockpit Local Info page; keyless place search (`/api/geocode/search`) — the fallback when no Google Maps key is configured, or when Google declines the Geocoding request | ODbL 1.0 + [Nominatim usage policy](https://operations.osmfoundation.org/policies/nominatim/) (identifying User-Agent, at most one request per second — both routes share one server-side queue) | "© OpenStreetMap contributors" |
