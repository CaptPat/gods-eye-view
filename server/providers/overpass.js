      ...
      try {
        ...
        const requestPromise = fetchOverpassPayload(safeBody)
          .then((payload) => {
            ...
            // outage that caused it. An empty element list is data but not
            // cacheable: a partial-coverage instance must not pin a layer empty.
            if (
              overpassPayloadIsData(payload) &&
              overpassPayloadIsCacheable(payload, cacheKey)
            ) {
